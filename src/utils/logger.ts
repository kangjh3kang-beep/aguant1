...
export function logStageStart(stage: ReviewStage): void {
    console.log(`${STAGE_ICONS[stage]} Starting...`);
}
export function logStageResult(stage: ReviewStage, status: StageStatus, duration: number): void {
    const label = STATUS_LABELS[status];
    console.log(`${STAGE_ICONS[stage]} ${label} (${duration}ms)`);
}
export function logIssue(severity: Severity, file: string, line: number | undefined, message: string): void {
    const label = SEVERITY_LABELS[severity];
    const loc = line !== undefined ? `${file}:${line}` : file;
    console.log(`  ${label}  ${loc}  ${message}`);
}
export function logHeader(projectPath: string): void {
    console.log('');
    console.log('═══════════════════════════════════════════════');
    console.log('  Antigravity Code Review Agent');
