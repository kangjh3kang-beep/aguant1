...
export class AutonomousLoop {
    ...
    constructor(project: ProjectSpec, pipelineConfig?: Partial<PipelineConfig>, autonomousConfig?: Partial<AutonomousConfig>) {
    ...
    run(): LoopState {
        ...
        while (this.state.cycle < this.config.maxHealingCycles) {
            ...
            if (this.config.multiModelEnabled) {
                const modelSelection = this.selectModelForTask(analysis.failedPhase || 'review', analysis.totalIssues, analysis.autoFixable.some((i) => i.severity === 'critical') ? 'critical' : 'normal');
            ...
            if (this.config.rootCauseAnalysis && analysis.totalIssues > 0) {
                ...
                for (const issue of allIssues.filter((i) => i.severity === 'error' || i.severity === 'critical').slice(0, 10)) {
                    const adaptive = this.adaptiveEngine.analyzeAndSelect(analysis.failedPhase || 'review', issue.message, issue.file ? [issue.file] : []);
                    if (adaptive.strategy) {
                        ...
                        // 전략 실행
                        const execResult = this.adaptiveEngine.executeStrategy(adaptive.strategy, adaptive.pattern, this.project.rootPath);
                        strategiesExecuted++;
            ...
            if (analysis.autoFixable.length > 0 && this.config.feedbackEnabled) {
                ...
                const feedbacks = this.buildSystemAwareFeedbacks(remainingIssues.length > 0 ? remainingIssues : analysis.autoFixable, pipelineResult);
                ...
                if (directResult.patchesApplied > 0) {
                    ...
                    for (const issue of analysis.autoFixable.slice(0, 20)) {
                        this.learningMemory.recordFix(issue.message, 'direct-patch', true, this.state.cycle);
                    }
                }
                ...
                const cleanupResult = this.codeTransformer.transformProject(['remove-unused-import', 'fix-empty-catch', 'remove-console']);
        ...
        this.learningMemory.updateProjectProfile(this.project.name, this.state.systemAnalysis?.techStack || [], this.state.summary.qualityScore);
    ...
    private buildFeedbacks(issues: TaskIssue[], pipelineResult: PipelineState): AgentFeedback[] {
        ...
        for (const [file, fileIssues] of byFile) {
            if (fileCount >= this.config.maxFixFilesPerCycle)
                break;
    ...
    private performSystemAnalysis(): SystemAnalysis {
        ...
        if (testCount === 0 && sourceCount > 5)
            problems.push('테스트가 없음 — 회귀 버그 위험');
        if (sourceCount > 0 && testCount / sourceCount < 0.3)
            problems.push(`테스트 비율 ${Math.round(testCount / Math.max(sourceCount, 1) * 100)}% — 커버리지 부족`);
        if (depCount > 100)
            problems.push(`의존성 ${depCount}개 — 공급망 위험`);
        const storyline = {
    ...
    private inferProjectType(components: string[], techStack: string[]): string {
        ...
        if (comps.some((c) => /frontend|app|pages|components/.test(c)) && comps.some((c) => /backend|api|server/.test(c)))
            return 'fullstack';
        if (comps.some((c) => /backend|api|server|routes|controllers/.test(c)))
            return 'backend-api';
        if (comps.some((c) => /src|app|pages|components/.test(c)) && techStack.some((t) => /React|Vue|Angular|Svelte/i.test(t)))
            return 'frontend-spa';
        if (comps.some((c) => /bin|cli/.test(c)))
            return 'cli';
        if (comps.some((c) => /packages|apps/.test(c)))
            return 'monorepo';
        return 'library';
    }
    private inferArchitecturePattern(components: string[]): string {
        const comps = components.map((c) => c.toLowerCase());
        if (comps.some((c) => /controllers/.test(c)) && comps.some((c) => /services/.test(c)))
            return 'MVC / Layered';
        if (comps.some((c) => /domain|entities/.test(c)) && comps.some((c) => /repositories|infrastructure/.test(c)))
            return 'Clean / DDD';
        if (comps.some((c) => /modules/.test(c)))
            return 'Modular';
        if (comps.some((c) => /packages|apps/.test(c)))
            return 'Monorepo / Micro-Frontend';
        return 'Standard';
    }
    ...
    // ─── 근본 원인 분석 ───
    private analyzeRootCauses(analysis: {
        totalIssues: number;
        autoFixable: TaskIssue[];
        failedPhase: TaskPhase | null;
    }, pipelineResult: PipelineState): RootCause[] {
        const causes: RootCause[] = [];
        const allIssues = pipelineResult.tasks.flatMap((t) => t.result?.issues || []);
        // 환경/설정 문제 감지 (DB 연결, 모듈 미설치 등)
        const envIssues = allIssues.filter((i) => /connection refused|module not found|cannot find module|ECONNREFUSED|no such file/i.test(i.message));
        ...
        const testMismatch = allIssues.filter((i) => /fixture.*not found|TypeError.*missing.*argument|unexpected keyword|assert.*!=|assert.*is not/i.test(i.message));
        ...
        // 인증/권한 로직 불일치
        const authIssues = allIssues.filter((i) => /401.*200|200.*401|unauthorized|forbidden/i.test(i.message));
        ...
        const archIssues = allIssues.filter((i) => /circular|god module|SRP|separation.*concern/i.test(i.message));
    ...
    private detectRecurringFailures(currentResult: PipelineState): string[] {
        if (this.state.pipelineResults.length < 2)
            return [];
        const recurring: string[] = [];
        const prevResult = this.state.pipelineResults[this.state.pipelineResults.length - 2];
        const prevIssueKeys = new Set(prevResult.tasks.flatMap((t) => t.result?.issues || [])
    ...
    private buildSystemAwareFeedbacks(issues: TaskIssue[], pipelineResult: PipelineState): AgentFeedback[] {
        ...
        if (this.state.systemAnalysis) {
            ...
            const rootCauseContext = this.state.rootCauses.length > 0
                ? '\n[근본 원인]\n' + this.state.rootCauses.map((rc) => `  - [${rc.category}] ${rc.description}\n    전략: ${rc.suggestedStrategy}`).join('\n')
                : '';
            // 반복 실패 패턴 주입
    ...
    private isExtendedAutoFixable(issue: TaskIssue): boolean {
        ...
        if (/unused.*import|declared but.*never (read|used)/.test(msg))
            return true;
        if (/console\.(log|debug|info)/.test(msg) && issue.file)
            return true;
        if (/no-explicit-any|unexpected any/.test(msg) && issue.file)
            return true;
        if (/empty.*catch|no-empty/.test(msg) && issue.file)
            return true;
        if (/hardcoded.*secret|hardcoded.*password/.test(msg) && issue.file)
            return true;
        if (/no-non-null-assertion/.test(msg) && issue.file)
            return true;
        // suggestion + line이 있으면 DirectFeedbackPipeline이 처리 가능
        if (issue.line && issue.suggestion && issue.file)
            return true;
        return false;
    }
     ...
     */
    private calculateQualityScore(pipelineResult: PipelineState, _analysis: {
        totalIssues: number;
        autoFixable: TaskIssue[];
        allPassed: boolean;
    }): number {
     ...
     */
    private selectModelForTask(phase: TaskPhase, issueCount: number, severity: string): {
        model: string;
        reason: string;
    } {
        if (!this.config.multiModelEnabled) {
     ...
     */
    private runLLMJudge(pipelineResult: PipelineState, qualityScore: number): {
        verdict: string;
        score: number;
        feedback: string;
    } {
     ...
     */
    private tryAIJudge(pipelineResult: PipelineState, qualityScore: number): {
        verdict: string;
        score: number;
        feedback: string;
    } | null {
        try {
            ...
            if (!aiConfig)
                return null;
            const allIssues = pipelineResult.tasks.flatMap((t) => t.result?.issues || []);
            const phaseSummary = pipelineResult.tasks.map((t) => `${t.phase}: ${t.status} (${t.result?.issues.length || 0}건)`).join('\n');
            const topIssues = allIssues.slice(0, 10).map((i) => `[${i.severity}] ${i.message}${i.file ? ` (${i.file})` : ''}`).join('\n');
...
수정 이력: ${this.state.fixHistory.length}건`;
     ...
     */
    private ruleBasedJudge(criticals: number, errors: number, warnings: number, passedTasks: number, totalTasks: number, qualityScore: number): {
        verdict: string;
        score: number;
        feedback: string;
    } {
