...
interface ErrorPattern {
    ...
    fix?: (file: string, error: string, _projectPath: string) => boolean;
}
        ...
        fix: (file: string, _error: string, projectPath: string): boolean => {
            ...
            try {
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[TesterAgent] file operation in timeout fix:', err instanceof Error ? err.message : String(err));
                }
            }
            return false;
        },
...
export class TesterAgent extends BaseSubAgent {
    ...
    protected executeTask(task: Task, projectPath: string): TaskResult {
        ...
        if (!testResult.passed) {
            this.addInsight('test-coverage', 'high', `테스트 실패: ${issues.filter((i) => i.severity === 'error').length}건`, issues.filter((i) => i.severity === 'error').map((i) => i.message).join('\n'), task, issues.map((i) => i.file || '').filter(Boolean));
        }
        // 커버리지 분석
        ...
        if (qualityResult.issues.length > 0) {
            this.addInsight('test-coverage', 'medium', `Test Smell ${qualityResult.issues.length}건, ${qualityResult.fixedCount}건 수정`, qualityResult.issues.map((i) => i.message).join('\n'), task, qualityResult.issues.map((i) => i.file || '').filter(Boolean));
        }
        // 커버리지 갭 분석 + 테스트 스텁 생성
        ...
        if (gapResult.issues.length > 0) {
            this.addInsight('test-coverage', 'medium', `커버리지 갭 ${gapResult.issues.length}건, 스텁 ${gapResult.generatedCount}개 생성`, gapResult.issues.map((i) => i.message).join('\n'), task, gapResult.issues.map((i) => i.file || '').filter(Boolean));
        }
     ...
     */
    private autoFixTestFailures(testIssues: TaskResult['issues'], projectPath: string): {
        logs: string[];
        fixedCount: number;
    } {
        const logs: string[] = [];
        ...
        for (const issue of testIssues) {
            ...
            for (const errorPattern of TEST_ERROR_PATTERNS) {
                if (!errorPattern.autoFixable || !errorPattern.fix)
                    continue;
                if (!errorPattern.pattern.test(issue.message) && !errorPattern.pattern.test(issue.suggestion || ''))
                    continue;
    ...
    private analyzeAndFixTestQuality(projectPath: string): {
        ...
        const scanDir = (dir: string, depth: number) => {
            ...
            try {
                    ...
                    if (entry.isDirectory()) {
                        ...
                        try {
                            ...
                            if (testBlocks > 0 && assertions < testBlocks) {
                                issues.push(this.createIssue('warning', `[Test Smell] ${relPath}: ${testBlocks}개 테스트 중 assertion 부족 (${assertions}개) — 모든 테스트에 assertion을 추가하세요`, { file: relPath, autoFixable: false }));
                                totalSmells++;
                            }
                            // Test Smell: Large test file (50+ tests)
                            if (testBlocks > 50) {
                                issues.push(this.createIssue('info', `[Test Smell] ${relPath}: ${testBlocks}개 테스트 — 테스트 파일을 분리하세요`, { file: relPath, autoFixable: false }));
                                totalSmells++;
                            }
                            // Test Smell: No describe block
                            if (testBlocks > 3 && !/describe\s*\(/.test(content)) {
                                issues.push(this.createIssue('info', `[Test Smell] ${relPath}: describe 블록 없음 — 테스트를 그룹화하세요`, { file: relPath, autoFixable: false }));
                                totalSmells++;
                            }
                            ...
                            if (hardcodedStrings && hardcodedStrings.length > 5) {
                                issues.push(this.createIssue('info', `[Test Smell] ${relPath}: 하드코딩된 테스트 데이터 과다 — factory/fixture 패턴을 사용하세요`, { file: relPath, autoFixable: false }));
                                totalSmells++;
                            }
                            ...
                            if (skipped && skipped.length > 0) {
                                ...
                                if (newContent !== content) {
                                    ...
                                    issues.push(this.createIssue('info', `[Test Smell] ${relPath}: 비활성 테스트 ${skipped.length}개 자동 활성화됨`, { file: relPath, autoFixable: true }));
                                }
                            }
                            if (fileModified) {
                                fs.writeFileSync(fullPath, content, 'utf-8');
                            ...
                            if (process.env.AG_DEBUG) {
                                console.debug('[TesterAgent] test file read/write:', err instanceof Error ? err.message : String(err));
                            }
                        }
                    }
                }
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[TesterAgent] test quality scan dir:', err instanceof Error ? err.message : String(err));
                }
            }
        };
    ...
    private analyzeAndFillCoverageGap(projectPath: string, framework: string): {
        ...
        const scanDir = (dir: string, depth: number) => {
            ...
            try {
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[TesterAgent] coverage gap scan dir:', err instanceof Error ? err.message : String(err));
                }
            }
        };
        scanDir(projectPath, 0);
        ...
        if (untestedFiles.length > 0) {
            ...
            for (const file of toGenerate) {
                ...
                if (generated) {
                    ...
                    issues.push(this.createIssue('info', `[Coverage Gap] ${file} — 테스트 스텁 자동 생성됨: ${generated}`, { file, autoFixable: true }));
                }
                else {
                    issues.push(this.createIssue('info', `[Coverage Gap] ${file} — 이 파일에 대한 테스트가 없습니다`, { file, autoFixable: false }));
                }
            }
            // 나머지는 보고만
     ...
     */
    private generateAITest(sourceFile: string, sourceContent: string, _projectPath: string, framework: string): string | null {
...
코드만 출력하세요. 설명은 불필요합니다.`;
    ...
    /** 일반 유닛 테스트 스텁 생성 */
    private generateUnitTestStub(importPath: string, importNames: string, exports: Array<{
        name: string;
        type: 'function' | 'class' | 'const' | 'type';
    ...
    private generateReactTestStub(importPath: string, exports: Array<{
        name: string;
        type: 'function' | 'class' | 'const' | 'type';
    }>, baseName: string): string {
     ...
     */
    private fixSourceIssuesFromKnowledge(projectPath: string, task: Task): {
        logs: string[];
        fixedCount: number;
    } {
        const logs: string[] = [];
        ...
        try {
            ...
            const cleanupResult = transformer.transformProject(['remove-unused-import', 'fix-empty-catch']);
            ...
            if (process.env.AG_DEBUG) {
                console.debug('[TesterAgent] CodeTransformer cleanup:', err instanceof Error ? err.message : String(err));
            }
        }
        return { logs, fixedCount };
    }
    private detectTestFramework(projectPath: string): string {
        ...
        if (fs.existsSync(pkgPath)) {
            try {
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[TesterAgent] package.json parse:', err instanceof Error ? err.message : String(err));
                }
            }
        }
    ...
    private runTests(projectPath: string, framework: string): {
        passed: boolean;
        logs: string[];
        issues: TaskResult['issues'];
    ...
    private parseJestOutput(output: string, logs: string[], issues: TaskResult['issues']): {
        passed: boolean;
        logs: string[];
        issues: TaskResult['issues'];
        coverageReport?: string;
        ...
        try {
            ...
            if (process.env.AG_DEBUG) {
                console.debug('[TesterAgent] Jest JSON parse fallback:', err instanceof Error ? err.message : String(err));
            }
        }
    ...
    private isAutoFixableTestError(errorMessage: string): boolean {
        return TEST_ERROR_PATTERNS.some((p) => p.autoFixable && p.pattern.test(errorMessage));
    }
}
