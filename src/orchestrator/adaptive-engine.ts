...
// ─── 실패 패턴 타입 ─────────────────────────────────────
export type FailureCategory = 'compile-error' | 'lint-error' | 'test-failure' | 'test-setup' | 'dependency-missing' | 'dependency-conflict' | 'config-error' | 'environment-error' | 'security-vulnerability' | 'type-error' | 'runtime-error' | 'integration-error' | 'architecture-violation' | 'permission-error' | 'network-error' | 'unknown';
...
export class FailurePatternDB {
     ...
     */
    record(category: FailureCategory, phase: TaskPhase, errorMessage: string, affectedFiles: string[] = []): FailurePattern {
    ...
    private save(): void {
        try {
            ...
            fs.writeFileSync(this.storagePath, JSON.stringify({ patterns: Array.from(this.patterns.values()) }, null, 2), 'utf-8');
        }
        catch (err: unknown) {
            if (process.env.AG_DEBUG) {
...
export class StrategySelector {
    ...
    selectStrategy(pattern: FailurePattern, excludeIds: string[] = []): FixStrategy | null {
        ...
        const matched = candidates.filter((s) => s.matchPatterns.some((mp) => {
            try {
                return new RegExp(mp, 'i').test(pattern.originalMessage);
            }
            catch (_err: unknown) {
                return false;
            }
...
export class AdaptiveEngine {
     ...
     */
    analyzeAndSelect(phase: TaskPhase, errorMessage: string, affectedFiles: string[] = []): {
     ...
     */
    executeStrategy(strategy: FixStrategy, pattern: FailurePattern, projectPath: string): {
        success: boolean;
        output: string;
        commands: string[];
    } {
        ...
        // 결과 기록
        this.recordResult(pattern.id, strategy.id, strategy.name, success, outputs.join('\n').slice(0, 500));
        this.logLearning(success ? 'execute-success' : 'execute-fail', `${strategy.name} → ${success ? 'SUCCESS' : 'FAIL'} (${commands.length} commands)`);
    ...
    recordResult(patternId: string, strategyId: string, strategyName: string, success: boolean, details: string): void {
        ...
        this.logLearning(success ? 'strategy-success' : 'strategy-fail', `${strategyName}: ${success ? 'SUCCESS' : 'FAIL'} - ${details.slice(0, 100)}`);
    }
    /**
     * 학습 보고서를 생성합니다.
     */
    ...
    private classifyFailure(message: string): FailureCategory {
        ...
        // 컴파일/타입 에러
        if (/ts\d{4}|type.*not assignable|cannot find name|property.*does not exist/i.test(lower))
            return 'type-error';
        if (/cannot find module|module not found|can't resolve/i.test(lower))
            return 'dependency-missing';
        if (/compilation failed|build.*failed|tsc.*error/i.test(lower))
            return 'compile-error';
        // 의존성
        if (/eresolve|peer dep|version.*conflict|incompatible/i.test(lower))
            return 'dependency-conflict';
        if (/no module named|modulenotfounderror/i.test(lower))
            return 'dependency-missing';
        // 테스트
        if (/econnrefused|connection.*refused|database.*unavailable/i.test(lower))
            return 'test-setup';
        if (/test.*fail|assert|expect.*receive|toequal|tobe/i.test(lower))
            return 'test-failure';
        if (/jest|mocha|pytest|test.*setup/i.test(lower))
            return 'test-setup';
        // 보안
        if (/vulnerability|cve-|advisory|hardcoded.*secret/i.test(lower))
            return 'security-vulnerability';
        // 환경
        if (/permission.*denied|eacces|eperm/i.test(lower))
            return 'permission-error';
        if (/econnrefused|timeout|network/i.test(lower))
            return 'network-error';
        if (/env.*not.*set|config.*missing/i.test(lower))
            return 'config-error';
        if (/command not found|not installed/i.test(lower))
            return 'environment-error';
        // 린트
        if (/eslint|prettier|lint.*error/i.test(lower))
            return 'lint-error';
        // 런타임
        if (/typeerror|referenceerror|syntaxerror|rangeerror/i.test(lower))
            return 'runtime-error';
        // 아키텍처
        if (/circular.*dependency|god.*class|too.*large/i.test(lower))
            return 'architecture-violation';
        // 통합
        if (/integration|api.*error|http.*\d{3}/i.test(lower))
...
class StrategyExecutor {
    ...
    execute(strategy: FixStrategy, pattern: FailurePattern): {
        success: boolean;
        commands: string[];
        outputs: string[];
    } {
    ...
    private handleDependencyOrCompile(strategy: FixStrategy, pattern: FailurePattern): {
        success: boolean;
        commands: string[];
        outputs: string[];
    } {
        ...
        if (strategy.id === 'compile-tsconfig-fix' || strategy.id === 'compile-type-fix') {
            ...
            try {
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[AdaptiveEngine] build exec error:', err instanceof Error ? err.message : String(err));
                }
            }
        }
        return { success: false, commands, outputs };
    }
    private handleLintError(strategy: FixStrategy): {
        success: boolean;
        commands: string[];
        outputs: string[];
    } {
        ...
        if (strategy.id === 'lint-autofix') {
            ...
            try {
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[AdaptiveEngine] eslint fix error:', err instanceof Error ? err.message : String(err));
                }
            }
    ...
    private handleTestIssue(strategy: FixStrategy, pattern: FailurePattern): {
        success: boolean;
        commands: string[];
        outputs: string[];
    } {
        ...
        if (strategy.id === 'test-snapshot-update') {
            ...
            try {
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[AdaptiveEngine] snapshot update error:', err instanceof Error ? err.message : String(err));
                }
            }
        }
            ...
            if (moduleMatch) {
                ...
                if (!moduleName.startsWith('.') && isValidNpmName) {
                    ...
                    try {
                        ...
                        if (process.env.AG_DEBUG) {
                            console.debug('[AdaptiveEngine] devDep install error:', err instanceof Error ? err.message : String(err));
                        }
                    }
                }
            }
    ...
    private handleDependencyConflict(_strategy: FixStrategy): {
        success: boolean;
        commands: string[];
        outputs: string[];
    } {
        ...
        try {
            ...
            if (process.env.AG_DEBUG) {
                console.debug('[AdaptiveEngine] dep conflict error:', err instanceof Error ? err.message : String(err));
            }
        }
        return { success: false, commands, outputs };
    }
    private handleSecurityVulnerability(strategy: FixStrategy, _pattern: FailurePattern): {
        success: boolean;
        commands: string[];
        outputs: string[];
    } {
        ...
        if (strategy.id === 'security-dep-update') {
            ...
            try {
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[AdaptiveEngine] npm audit fix error:', err instanceof Error ? err.message : String(err));
                }
            }
        }
    ...
    private handleTypeError(_strategy: FixStrategy): {
        success: boolean;
        commands: string[];
        outputs: string[];
    } {
        ...
        try {
            ...
            if (process.env.AG_DEBUG) {
                console.debug('[AdaptiveEngine] type check error:', err instanceof Error ? err.message : String(err));
            }
        }
        return { success: transformResult.changed > 0, commands, outputs };
    }
    private handleArchitectureViolation(strategy: FixStrategy, _pattern: FailurePattern): {
        success: boolean;
        commands: string[];
        outputs: string[];
    } {
    ...
    private handleWithCodeTransformer(_strategy: FixStrategy, pattern: FailurePattern): {
        success: boolean;
        commands: string[];
        outputs: string[];
    } {
