...
import { BrowserAutomation, BrowserAutomationConfig, BrowserAutomationResult, UserFlow, } from '../browser-automation';
export class BrowserAgent extends BaseSubAgent {
    ...
    protected getCapabilities(): string[] {
        ...
        if (this.checkPuppeteerRuntime()) {
            capabilities.push('real-browser-automation', 'dev-server-management', 'screenshot-capture', 'visual-regression', 'console-error-detection', 'responsive-testing', 'core-web-vitals-measurement', 'performance-metrics', 'user-flow-execution', 'page-interaction', 'auto-link-discovery', 'dom-a11y-audit');
        }
    ...
    protected executeTask(task: Task, projectPath: string): TaskResult {
        ...
        if (puppeteerAvailable) {
            ...
            if (automationResult) {
                ...
                for (const page of automationResult.pages) {
                    if (page.screenshot)
                        artifacts.push(page.screenshot);
                }
        ...
        if (a11yIssues.length > 0) {
            this.addInsight('accessibility', 'medium', `접근성 이슈 ${a11yIssues.length}건`, a11yIssues.map((i) => i.message).join('\n'), task, a11yIssues.map((i) => i.file || '').filter(Boolean));
        }
        const perfIssues = issues.filter((i) => i.message.includes('Performance') || i.message.includes('LCP') || i.message.includes('CLS'));
        if (perfIssues.length > 0) {
            this.addInsight('recommendation', 'medium', `성능 이슈 ${perfIssues.length}건`, perfIssues.map((i) => i.message).join('\n'), task);
        }
        const browserErrors = issues.filter((i) => i.severity === 'critical' || i.severity === 'error');
        if (browserErrors.length > 0) {
            this.addInsight('accessibility', 'high', `브라우저 에러 ${browserErrors.length}건`, browserErrors.map((i) => i.message).join('\n'), task);
        }
        const consoleIssues = issues.filter((i) => i.message.includes('Console'));
        if (consoleIssues.length > 0) {
            this.addInsight('recommendation', 'high', `콘솔 에러 ${consoleIssues.length}건`, consoleIssues.map((i) => i.message).join('\n'), task);
        }
        ...
        if (fs.existsSync(flowConfigPath)) {
            try {
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[BrowserAgent] invalid flow config:', err instanceof Error ? err.message : String(err));
                }
            }
        }
        // 기본 탐색 플로우 (항상 포함)
...
`;
            ...
            if (process.env.AG_DEBUG) {
                console.debug('[BrowserAgent] automation sync setup:', err instanceof Error ? err.message : String(err));
            }
        }
        return null;
    }
    // ─── 정적 분석 모드 (Puppeteer 없을 때 fallback) ────────
    private runStaticAnalysis(projectPath: string, outputs: string[], issues: TaskIssue[], artifacts: string[], screenshotDir: string): void {
    ...
    private checkPuppeteerPackage(projectPath: string): boolean {
        try {
            ...
            if (process.env.AG_DEBUG) {
                console.debug('[BrowserAgent] puppeteer package check:', err instanceof Error ? err.message : String(err));
            }
        }
        return false;
    }
    private runPuppeteerTests(projectPath: string, screenshotDir: string): {
        logs: string[];
        issues: TaskIssue[];
        artifacts: string[];
    } {
    ...
    private runBasicVerification(projectPath: string): {
        logs: string[];
        issues: TaskIssue[];
    } {
        const logs: string[] = [];
    ...
    private checkAccessibility(projectPath: string): {
        logs: string[];
        issues: TaskIssue[];
    } {
        const logs: string[] = [];
        ...
        const scanDir = (dir: string, depth: number) => {
            ...
            try {
                ...
                for (const entry of entries) {
                    ...
                    if (entry.isDirectory()) {
                        ...
                        try {
                            ...
                            if (process.env.AG_DEBUG) {
                                console.debug('[BrowserAgent] a11y file read:', err instanceof Error ? err.message : String(err));
                            }
                        }
                    }
                }
                ...
                if (process.env.AG_DEBUG) {
                    console.debug('[BrowserAgent] a11y scan dir:', err instanceof Error ? err.message : String(err));
                }
            }
        };
    ...
    private safeUnlink(filePath: string): void {
        try {
            ...
            if (process.env.AG_DEBUG) {
                console.debug('[BrowserAgent] safe unlink:', err instanceof Error ? err.message : String(err));
            }
        }
    }
}
